			...
			<div class="mt mb">
				...
				<?php $this->load->view('hr/top_view');?>
					<?php if($this->session->userdata('user_type') != 'EMP' ){ ?>
						<a href="<?= base_url('en/hr/comp_profile_readonly_emp'.$get_id);?>" class="btn btn-primary pull-right" role="button" style="margin-right: 10px; margin-top: 4px;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;View</a> 
					<?php } ?>
					<legend class="pkheader">Company</legend>
					<div class="row well">
						<div class="table-responsive">
							<form id="frmCompanyUpdate" name="frmCompanyUpdate" method="POST" action="<?= base_url('en/hr/comp_profile_update_emp'.$get_id);?>" enctype="multipart/form-data"> 
							<table class="table table-striped table-bordered table-condensed"> 
								<tbody>
									<tr>
										...
										<select  name="reporting" id="reporting"  class="selectpicker form-control required" data-live-search="true" required="" style="width: 95%" >
											<?php 
											for($l=0; $l < count($getReportingManager); $l++) 
											{?>
												<option value="<?php echo $getReportingManager[$l]['login_id']; ?>" <?php if($getReportingManager[$l]['login_id'] == $empInfo[0]['reporting_to']){ echo "selected"; }?> ><?php echo $getReportingManager[$l]['dispName']; ?></option>	
											<?php } ?>
										</select>
                                            </td>	
