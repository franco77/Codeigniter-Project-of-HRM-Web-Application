...
<div class="section main-section" ng-app="myApp" ng-controller="midYear" ng-init="init('<?php echo base_url() ?>')">
    <div class="container"> 
		<div class="form-content page-content">
			<div class="mt mb">
				...
				<div class="col-md-9 center-xs">
					...
					<legend class="pkheader">Mid Year Review Detail ({{totalItems}}) <small>(Resource of our Organisation)</small>
					...
					<div class="row well">
						...
						<div class="row pkdsearch">
							...
							<div class="col-md-5">
								<div class="form-group row">
									...
									<div class="col-sm-8">
										<select name="searchYear" id="searchYear" ng-model="searchYear" class="search_UI form-control input-sm">
											<?php
											  $m = date('m');
												$y = date('Y');
												if($m >=4){
						...
						<div class="table-responsive" ng-show="filteredItems > 0">
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody>
									<tr ng-repeat="data in filtered = (getMidYear | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
										<td>{{$index+1}}</td>
										<td>{{data.name}}</a> </td>
										<td>{{data.apply_date}}</a> </td>
										<td>{{data.approved_date}}</td>
										<td ng-if="data.rm_status == '0' " align="center">Pending</td>
										...
										<td><textarea name="remark"  rows="1" style="width: 110px; max-width:110px;" ng-keyup="updateRemark($event, data.mid)" >{{data.remark}}</textarea></td>
									</tr>
								</tbody>
							</table>
							<div class="row"> 
