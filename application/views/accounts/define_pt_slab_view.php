				...
				<div class="col-md-9 center-xs">
					...
					<div class="row well">
						<div class="row pkdsearch">
							 <legend class="form_title col-md-12"><span id="formType"></span>Add/Edit Professional Tax Slab</strong> </legend>
							<div class="form">
								<div class="form1">
									<form id="frmUpdateCountry" name="frmUpdateCountry" method="POST">
											...
											<div class="col-md-5" style="padding-left: 0px;padding-right: 0px;">
												<div class="form-group">
													<label for="staticEmail" class="col-sm-5 col-form-label">PT Slab Range</label>
													<div class="col-sm-7 row">
														<input type="text" class=" form-control" id="from_range" placeholder="from" required name="from_range" />
											...
											<div class="col-md-3" style="padding-left: 0px;padding-right: 0px;">
												<div class="form-group">
													<div class="col-sm-10 row">
														<input type="text" class=" form-control" id="to_range" placeholder="to"  required name="to_range" />
													</div>
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label for="staticEmail" class="col-sm-4 col-form-label">PT Value</label>
													<div class="col-sm-8">
														<input type="text" class=" form-control" id="pt_value" required placeholder="value"   name="pt_value" />
													</div>
												</div>
											</div>
							...
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody id="secVal">
									...
									<tr>
										...
										<td><?= $v1['range'] ?></td>
										<td><?= $v1['pt_value'] ?></td>
										<td><a onclick="delete_(<?= $v1['pt_id'] ?>)"><i class="fa fa-trash"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="modify_('<?= $v1['pt_id'] ?>','<?= $v1['range'] ?>','<?= $v1['pt_value'] ?>')"><i class="fa fa-pencil"></i></a></td>
									</tr>
									<?php $i++; endforeach; ?>
								</tbody>
							</table>
...
<script type="text/javascript">
    ...
    frm.submit(function (e) {
        ...
        $.ajax({
            ...
            url: site_url+'en/accounts_admin/add_edit_ptslab',
            data: frm.serialize(),
            success: function (data) {
                console.log(data);
				$('#piMSG').html('Professional Tax Slab submitted Successfully');
	...
	function delete_(dat){
		$.ajax({
			type: 'POST',
			url:site_url+'en/accounts_admin/delete_pt_slab',
			data:{pt_id:dat},
			success:function(data){
				$('#piMSG').html('Professional Tax Slab deleted Successfully');
				 $("#listpayroll").load(" #listpayroll > *");	
			}
		});
	}
	...
	function getFYData(dis){
		...
		$.ajax({
			type: 'POST',
			url:site_url+'en/accounts_admin/get_proffessional_tax_slab_fy',
			...
			success:function(data){
				...
				for (var i=0; i<data.length; i++){
					...
					str += '<td>'+data[i].pt_value+'</td>';
					str += '<td><a onclick="delete_('+data[i].pt_id+')"><i class="fa fa-trash"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="modify_('+data[i].pt_id+',\''+data[i].range+'\',\''+data[i].pt_value+'\')"><i class="fa fa-pencil"></i></a></td>';
					str += '</tr>';
				}
				$('#secVal').html(str);
			}
