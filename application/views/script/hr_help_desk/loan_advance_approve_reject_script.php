...
myapp.controller('allLoanAdvance', ['$http','$scope', '$timeout',  '$window', function ($http, $scope,$timeout,$window) 
{   
    $scope.init = function(base_url,data)
    {
        $scope.base_url=base_url; 
        $http.get($scope.base_url+'Hr_help_desk/get_all_loan_advance_approve_reject').
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/Hr_help_desk/get_all_loan_advance_approve_reject_search',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
        ...
        $scope.loanApporved = function(lid) {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr_help_desk/update_loan_advance_approved_rm',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
        ...
        $scope.loanRejected = function(lid) {
			...
			if($('#'+reason).val() !=""){
				...
				$http({
					method: 'POST', 
					url: $scope.base_url+'en/hr_help_desk/update_loan_advance_rejected_rm',
					headers: {'Content-Type': undefined},  
					data: formdata
				}).
				success(function(response) {
		...
		//loanApporved
        $scope.loanApporvedDH = function(lid) {
			var formdata = new FormData();
			formdata.append('lid', lid);
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr_help_desk/update_loan_advance_approved_dh',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
		...
		//loanRejected
        $scope.loanRejectedDH = function(lid) {
			var formdata = new FormData();
			formdata.append('lid', lid);
			var reason = 'reason_'+lid;
			formdata.append('reject_reason', $('#'+reason).val());
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr_help_desk/update_loan_advance_rejected_dh',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
