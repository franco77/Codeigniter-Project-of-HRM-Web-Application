...
{   
    $scope.init = function(base_url,data)
    {
		...
		var yr1 = yr+1;
		$scope.searchYear = yr+"-"+yr1;console.log($scope.searchYear);console.log(yy);
        $http.get($scope.base_url+'accounts_admin/get_all_final_declaration').
		...
		$scope.openNewWindow = function(tid,login_id) {
            $window.open($scope.base_url+'accounts_help_desk/my_estimated_declaration_print?id='+login_id+'&tid='+tid, 'NewWindow', 'width=980,height=600,left=0,top=0,scrollbars=1');
        };
		...
		$scope.getFYData = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/accounts_admin/get_all_final_declaration_fy',
