...
{   
    $scope.init = function(base_url,data)
    {
        ...
        $http.get($scope.base_url+'accounts_admin/get_all_final_tax_computation').
		...
		$scope.openNewWindow = function(tid,login_id) {
            $window.open($scope.base_url+'accounts_help_desk/final_tax_compution_sheet_result_print?id='+login_id+'&tid='+tid, 'NewWindow', 'width=980,height=600,left=0,top=0,scrollbars=1');
        };
		...
		$scope.getFYData = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/accounts_admin/get_all_final_tax_computation_fy',
