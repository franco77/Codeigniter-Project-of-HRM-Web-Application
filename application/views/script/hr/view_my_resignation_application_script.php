...
{   
    ...
    {
		var date = new Date();
		var current_year = date.getFullYear();//console.log(current_year);
		$scope.year = current_year.toString();
        $scope.base_url=base_url; 
        $http.get($scope.base_url+'en/hr/get_all_resignation_application_details').
		...
		$scope.advanceFilter = function() {
			...
			formdata.append('searchYear', ($scope.year)? $scope.year : "");
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/get_all_resignation_application_details_search',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
		...
		//resignationApporved
        $scope.resignationApporved = function(rid) {
			var formdata = new FormData();
			formdata.append('rid', rid);
			var reason = 'remarks_'+rid;
			formdata.append('remarks', $('#'+reason).val());
			//alert($('#'+reason).val());
			if($('#'+reason).val() !=""){
				$http({
					method: 'POST', 
					url: $scope.base_url+'en/hr/update_resignation_application_approved',
					headers: {'Content-Type': undefined},  
					data: formdata
				}).
				success(function(response) {
				...
				$('#'+reason).attr('style', 'border-color: #f00000;');
			}
        };
        ...
        $scope.resignationRejected = function(rid) {
			...
			if($('#'+reason).val() !=""){
				$http({
					method: 'POST', 
					url: $scope.base_url+'en/hr/update_resignation_application_rejected',
					headers: {'Content-Type': undefined},  
					data: formdata
				}).
				success(function(response) {
