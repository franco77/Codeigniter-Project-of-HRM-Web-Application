...
myapp.controller('profileList', function ($http, $scope,$timeout, $window) 
{   
    $scope.init = function(base_url,data)
    {
        $scope.base_url=base_url; 
        $http.get($scope.base_url+'en/hr/get_active_employee').
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/home/get_all_active_emp_advance_search',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
		...
		$scope.exportBioAttendance = function() {
			
			var searchDepartment = ($scope.searchDepartment)? $scope.searchDepartment : "";
			var searchName = ($scope.searchName)? $scope.searchName : "";
			var searchDesignation = ($scope.searchDesignation)? $scope.searchDesignation : "";
			var searchEmpCode = ($scope.searchEmpCode)? $scope.searchEmpCode : "";
			var aaDetais = "";
			
			aaDetais = "?department="+searchDepartment+"&name="+searchName+"&designation="+searchDesignation+"&empcode="+searchEmpCode;
			$window.open( $scope.base_url+'en/hr/get_user_info'+aaDetais , '_blank');
        };
