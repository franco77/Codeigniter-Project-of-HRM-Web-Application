...
{   
    $scope.init = function(base_url,data)
    {
		...
		//get default datas
        $http.get($scope.base_url+'en/hr/get_loan_advance_approve_reject').
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/get_loan_advance_approve_reject_search',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
        ...
        $scope.loanApporved = function(lid) {
			...
			var approveamount = 'txtApproveAmount_'+lid;
			formdata.append('approveamount', $('#'+approveamount).val());
			var approveinstalment = 'txtinstalment_'+lid;
			formdata.append('approveinstalment', $('#'+approveinstalment).val());
			var selMonth = 'selMonth_'+lid;
			formdata.append('selMonth', $('#'+selMonth).val());
			var selYear = 'selYear_'+lid;
			formdata.append('selYear', $('#'+selYear).val());
			if($('#'+approveamount).val() !="" && $('#'+approveinstalment).val() !="" && $('#'+selMonth).val() !="" && $('#'+selYear).val() !=""){
				$http({
					method: 'POST', 
					url: $scope.base_url+'en/hr/update_loan_advance_approved_hr',
					headers: {'Content-Type': undefined},  
					data: formdata
				}).
				success(function(response) {
        ...
        $scope.loanRejected = function(lid) {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/update_loan_advance_rejected_hr',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
