...
myapp.controller('resumeFormat', function ($http, $scope,$timeout) 
{   
    $scope.init = function(base_url,data)
    {
        $scope.base_url=base_url; 
        $http.get($scope.base_url+'en/hr/get_active_emp_resume').
        success(function(data, status)
        {  
            $scope.getresume = data;
            //$scope.data.dt_event_date = Date.parse($scope.data.dt_event_date);
            $scope.currentPage = 1; //current page
            $scope.entryLimit = 10; //max no of items to display in a page
            $scope.filteredItems = $scope.getresume.length; //Initially for no filter  
            $scope.totalItems = $scope.getresume.length;
        })
        .
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/home/get_all_emp_advance_search',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
				$scope.getresume = response;
				$scope.currentPage = 1; //current page
				$scope.entryLimit = 10; //max no of items to display in a page
				$scope.filteredItems = $scope.getresume.length; //Initially for no filter  
				$scope.totalItems = $scope.getresume.length;
