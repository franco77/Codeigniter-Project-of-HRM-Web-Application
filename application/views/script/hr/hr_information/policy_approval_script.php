...
myapp.controller('holidaylist', function ($http, $scope,$timeout,$window) 
{   
    $scope.init = function(base_url,data)
    {
        ...
        $http.get($scope.base_url+'en/hr/get_list_policy_approval').
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/get_list_policy_approval_search',
		...
		$scope.delete_ = function(val) {
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/delete_policy_approval',
		...
		$scope.addPolicy = function() {
			var formdata = new FormData();
			formdata.append('policy_title', ($scope.policy_title)? $scope.policy_title : "");
			formdata.append('editor1', ($scope.editor1)? $scope.editor1 : "");
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/hr/add_policy_approval',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
