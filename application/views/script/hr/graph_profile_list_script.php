...
myapp.controller('profileList', function ($http, $scope,$timeout) 
{   
    $scope.init = function(base_url,data)
    {
        $scope.base_url=base_url; 
        $http.get($scope.base_url+'en/hr/get_all_employee').
		...
		$scope.getStatus = [{ name: 'Active', val: '1'},{ name: 'Inactive', val: '2'}]; 
		...
		$scope.advanceFilter = function() {
			...
			$http({
				method: 'POST', 
				url: $scope.base_url+'en/home/get_all_emp_advance_search_filter',
				headers: {'Content-Type': undefined},  
				data: formdata
			}).
			success(function(response) {
