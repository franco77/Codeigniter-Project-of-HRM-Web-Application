							...
							<div class="form1 multiSelectHolder">
								...
								<table class="table table-striped table-bordered table-condensed" id="dataTable">
									<tbody style="background: #fff;">
										<tr class="info">
											<td width="5%">&nbsp;</td>
											<td width="40%"><strong>Performance Objectives<strong></td>
											<td width="35%"><strong>Target<strong></td>
											<td width="10%"><strong>Weightage<strong></td>
										...
										<tr>
											...
											<td><textarea name="objective[]" class="form-control" rows="4" <?php if($qryInfo[$i]['rm_status'] == '1') { echo 'disabled'; } ?> ><?php echo $qryInfo[$i]['objective']; ?></textarea></td>
											<td><textarea name="target[]" class="form-control" rows="4" <?php if($qryInfo[$i]['rm_status'] == '1') { echo 'disabled'; } ?>><?php echo $qryInfo[$i]['target']; ?></textarea></td>
											<td valign="top">
												<select name="weightage[]" class="form-control weightageBaby" <?php if($qryInfo[$i]['rm_status'] == '1') { echo 'disabled'; } ?>>
													<option value="0">Select</option>       
													<?php for($j=1; $j<=100; $j++){ ?>
													<option value="<?php echo $j; ?>" <?php if($qryInfo[$i]['weightage']==$j) echo "selected"; ?>><?php echo $j; ?></option>
													<?php } ?>                                                    
												</select> 
												<input  name="mid[]" type="hidden" value = "<?php echo $qryInfo[$i]['mid']; ?>">
												<input  name="mids[]" type="hidden" value = "<?php echo $qryInfo[$i]['mid']; ?>">
												<input  name="rm_status[]" type="hidden" value = "<?php echo $qryInfo[$i]['rm_status']; ?>">
											
											</td>
											<td valign="top">
												<select name="progress[]" class="form-control progressBaby" >
													<option value="0">Select</option>       
													<?php for($j=$qryInfo[$i]['progress']; $j<=100; $j++){ ?>
													<option value="<?php echo $j; ?>" <?php if($qryInfo[$i]['progress']==$j) echo "selected"; ?>><?php echo $j; ?></option>
													<?php } ?>        
												</select>
											</td>
										...
										<tr>
											...
											<td><textarea name="objective[]" class="form-control" rows="4"></textarea></td>
											<td><textarea name="target[]" class="form-control" rows="4"></textarea></td>
											<td valign="top">
												<select name="weightage[]" class="form-control weightageBaby">
													<option value="0">Select</option>
								...
								<div class="row">
									<div class="col-md-4 pull-left">
										<span class="pull-left">
											<input value="Add Row" onclick="addRow('dataTable')" type="button" class="btn btn-sm btn-info" <?php if(count($qryInfo) > 0){ if($qryInfo[0]['rm_status'] == '1') { echo 'disabled'; } } ?>> 
											<input value="Delete Row" onclick="deleteRow('dataTable')" type="button" class="btn btn-sm btn-info" <?php if(count($qryInfo) > 0){ if($qryInfo[0]['rm_status'] == '1') { echo 'disabled'; } } ?>>
										</span>
									</div>
