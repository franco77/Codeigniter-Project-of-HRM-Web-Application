						...
						<div class="row pkdsearch">
							...
							<div class="col-md-5">
								<div class="form-group row">
									...
									<div class="col-sm-8">
										<select ng-model="searchStatus" name="searchStatus" id="searchStatus" class="form-control input-sm"> 
											<option value="" >Select</option>
											<option ng-repeat="searchStatus in getStatus" ng-selected="{{ searchStatus.selected == true }}" selected=""  value="{{ searchStatus.val }}">{{ searchStatus.name }}</option>
										</select>
									</div>
								</div>
							</div>
							...
							<table class="table table-striped table-bordered table-condensed table-hover">
								...
								<tbody>
									<tr ng-repeat="data in filtered = (getprofilelist | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
										<td>{{$index+1}}</td>
										<td><a ng-href="<?php echo base_url(); ?>en/management/ctc_graph?id={{data.login_id}}" ng-bind="data.name;" ></a> </td>
