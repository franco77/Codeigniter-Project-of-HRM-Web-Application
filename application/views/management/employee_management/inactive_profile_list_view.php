...
<div class="section main-section" ng-app="myApp" ng-controller="inactiveList" ng-init="init('<?php echo base_url() ?>')">
    <div class="container"> 
		<div class="form-content page-content">
			<div class="mt mb">
				...
				<div class="col-md-9"> 
					<legend class="pkheader">All Inactive Employee Details({{ totalItems}})</legend>  
					<div class="row well">
					
						<div class="row pkdsearch">
							 <legend class="form_title col-md-12">Track All Inactive Employee </legend>
							<div class="col-md-5">
							...
							<table class="table table-striped table-bordered table-condensed table-hover">
								...
								<tbody>
									<tr ng-repeat="data in filtered = (getprofilelist | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
										<td>{{$index+1}}</td>
										<td><a ng-href="<?php echo base_url(); ?>en/management/general_readonly?id={{data.login_id}}" ng-bind="data.name;" ></a> </td>
										<td>{{data.loginhandle}}</td>
										<td>{{data.email}}</td> 
										<td>{{data.desg_name}}</td>
										<td ng-if="data.user_status == 1"><span class="label label-info">Active</span></td> 
										<td ng-if="data.user_status == 2"><span class="label label-danger">Inactive</span></td> 
										<td ng-if="data.user_status == 3"><span class="label label-danger">Inactive</span></td> 
										<td ng-if="data.user_status == 0"><span class="label label-danger">Inactive</span></td> 
									</tr>
