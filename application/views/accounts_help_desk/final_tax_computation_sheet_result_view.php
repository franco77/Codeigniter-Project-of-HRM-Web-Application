...
<table width="100%" class="table-noborder">
	<tr>
		<td colspan="4"><h3><center>Final Tax Computation Sheet For FY <?php echo (string)$fyear.'-'.(string)($fyear+1); ?></center></h3></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><?php echo $doj = date("d-m-Y", strtotime($empInfo['join_date']));
	...
	<tr>
		<td style="border-right: none">
			<table width="95%">
					...
					<tr>
						<td>Basic</td>
						<td><?php echo $basic_est = number_format((float)($earned_basic_total), 2, '.', ''); ?></td>
						<td><?php echo $basic_ded = number_format((float)(0.00), 2, '.', ''); ?></td>
						<td><?php echo $basic_tax = number_format((float)($earned_basic_total), 2, '.', ''); ?></td>
					</tr>
					<tr>
						<td>HRA</td>
						<td><?php echo $hra_est = number_format((float)($earned_hra_total), 2, '.', ''); ?></td>
						<td><?php 
								$actual_hra_rec = number_format((float)($earned_hra_total), 2, '.', '');
								$per_basic1 = number_format((float)($earned_basic_total/100)*40, 2, '.', '');
								$per_basic2 = number_format((float)($earned_basic_total/100)*50, 2, '.', '');
								$rent_paid_over_basic = number_format((float)($empInfo['eligible_rent_paid_by_employee'])-($earned_basic_total/100)*10, 2, '.', '');
						...
						<td><?php
								if($empInfo['eligible_rent_paid_by_employee'] == 0)
								{
								echo $hra_tax = number_format((float)($earned_hra_total)-0.00, 2, '.', '');
								}
								elseif($userInfo[0]['state_region2'] ==26)
								{
								echo $hra_tax = number_format((float)($earned_hra_total)-$minValue2, 2, '.', '');
								}
								else
								{
								echo $hra_tax = number_format((float)($earned_hra_total)-$minValue1, 2, '.', '');
								}
							?>
						</td>
					</tr>
					<tr>
						<td>Medical Exp.</td>
						<td><?php echo $medical_est = number_format((float)($earned_medical_allowance_total), 2, '.', '')?></td>
						<td><?php
								if($earned_medical_allowance_total == 0)
								{
						...
						<td><?php
								if($earned_medical_allowance_total == 0)
								{
									echo $medical_tax = number_format((float)($earned_medical_allowance_total-0.00), 2, '.', '');
								}
								else
								{
									echo $medical_tax = number_format((float)($earned_medical_allowance_total-$empInfo['eligible_medicalexpensesperannum']), 2, '.', '');
								} 
							?>
						</td>
					</tr>
					<tr>
						<td>Conv Allow</td>
						<td><?php echo $conv_est = number_format((float)($earned_conveyance_allowance_total), 2, '.', ''); ?></td>
						<td><?php echo $conv_ded = number_format((float)($empInfo['eligible_conv_allowance']), 2, '.', ''); ?></td>
						<td><?php if(($earned_conveyance_allowance_total-$empInfo['eligible_conv_allowance'])>0){ 
							echo $conv_tax = number_format((float)($earned_conveyance_allowance_total-$empInfo['eligible_conv_allowance']), 2, '.', ''); 
						}else{ echo $conv_tax = number_format((float)(0.00), 2, '.', ''); } 
						?></td>
					</tr>
					<tr>
						<td>Special Allow</td>
						<td><?php echo $special_est = number_format((float)($earned_special_allowance_total), 2, '.', '')?></td>
					...
					<tr>
						...
						<td colspan="2"><?php echo $hra_received = number_format((float)($earned_hra_total), 2, '.', '')?></td>
					</tr>
					<tr>
						...
						<td colspan="2"><?php 
							...
							{
								echo $rent_basic = number_format((float)($earned_basic_total/100)*50, 2, '.', '');
							} 
							else
							{
								echo $rent_basic = number_format((float)($earned_basic_total/100)*40, 2, '.', '');
							}
					...
					<tr>
						...
						<td colspan="2"><?php
							...
							{
								echo $rent_paid_10_basic = number_format((float)($empInfo['eligible_rent_paid_by_employee'])-($earned_basic_total/100)*10, 2, '.', '');
							} 
						?></td>
					</tr>
					<tr>
					  ...
					  <tr>
						...
						<tr>
							...
							<td><?php
								...
								$total_income = number_format((float)($earned_basic_total + $earned_hra_total + $earned_medical_allowance_total + $earned_conveyance_allowance_total + $earned_special_allowance_total + $empInfo['eligible_childreneducationalallowance'] + $empInfo['otherincome']), 2, '.', '');
...
<table width="100%" class="table-noborder">
	<tr>
		<td colspan="4"><h3><center>No record found</center></h3></td>
	</tr>
</table>
<?php } ?>
...
<?php /* } else { echo "<br><br><br><br><center><b><h3><span class='red'>No Record Found for this Employee.</span></h3></b><center>";} */ ?>