...
class MY_Controller extends MX_Controller {
	...
	private function _setup()
	{
		...
		$this->mAdmin_menu = empty($config['admin_menu']) ? array() : $config['admin_menu'];
		$this->mAdministrator_menu = empty($config['administrator_menu']) ? array() : $config['administrator_menu'];
		$this->mHrr_menu = empty($config['hrr_menu']) ? array() : $config['hrr_menu'];
		$this->mManagementt_menu = empty($config['managementt_menu']) ? array() : $config['managementt_menu'];
		$this->mAc_menu = empty($config['ac_menu']) ? array() : $config['ac_menu'];
		$this->mEmp_menu = empty($config['emp_menu']) ? array() : $config['emp_menu'];
		$this->mTimesheet_sidebar = empty($config['timesheet_sidebar']) ? array() : $config['timesheet_sidebar'];
		$this->mEvents = empty($config['events']) ? array() : $config['events'];		
		$this->mResources = empty($config['resources']) ? array() : $config['resources'];		
		$this->mIcompass_help = empty($config['icompass_help']) ? array() : $config['icompass_help'];
		$this->mHr_menu = empty($config['hr_menu']) ? array() : $config['hr_menu'];		
		$this->mHr_help_desk_menu = empty($config['hr_help_desk_menu']) ? array() : $config['hr_help_desk_menu'];
		$this->mAccounts = empty($config['accounts']) ? array() : $config['accounts'];
		$this->mAccounts_menu = empty($config['accounts_menu']) ? array() : $config['accounts_menu'];
		$this->mAabsys_classified_menu = empty($config['aabsys_classified_menu']) ? array() : $config['aabsys_classified_menu'];
		$this->mMy_account = empty($config['my_account']) ? array() : $config['my_account'];		
	...
	protected function render($view_file, $layout = 'default')
	{
		...
		$this->mViewData['admin_menu'] = $this->mAdmin_menu;
		$this->mViewData['administrator_menu'] = $this->mAdministrator_menu;
		$this->mViewData['hrr_menu'] = $this->mHrr_menu;
		$this->mViewData['managementt_menu'] = $this->mManagementt_menu;
		$this->mViewData['ac_menu'] = $this->mAc_menu;
		$this->mViewData['emp_menu'] = $this->mEmp_menu;
		$this->mViewData['timesheet_sidebar'] = $this->mTimesheet_sidebar;
		$this->mViewData['events'] = $this->mEvents;
		$this->mViewData['accounts'] = $this->mAccounts;
		$this->mViewData['accounts_menu'] = $this->mAccounts_menu;
		$this->mViewData['resources'] = $this->mResources;
		$this->mViewData['hr_menu'] = $this->mHr_menu;
		$this->mViewData['hr_help_desk_menu'] = $this->mHr_help_desk_menu;
		$this->mViewData['icompass_help'] = $this->mIcompass_help;
		$this->mViewData['aabsys_classified_menu'] = $this->mAabsys_classified_menu;
		$this->mViewData['my_account'] = $this->mMy_account;
